...
def find_most_efficient_vehicles_for_number_of_people(vehicles, distance, number_of_people, number_of_days, average_cost_of_gas_per_gallon):
    combinations = valid_combinations_of_vehicles(vehicles, number_of_people)
...
def valid_combinations_of_vehicles(vehicles, number_of_people):
    ...
    for index in range(len(vehicles)):
        vehicles_copy = list.copy(vehicles)
        vehicles_copy.pop(index)
        result += valid_combinations_of_vehicles(vehicles_copy, number_of_people)
    return result
